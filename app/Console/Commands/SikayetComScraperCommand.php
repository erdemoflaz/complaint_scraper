...
class SikayetComScraperCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'sikayet_com:scraper';
    ...
    public function handle()
    {
        ...
        $fromPage = 1;
        $toPage = 6; // last page is 57

        for ($i = $fromPage; $i <= $toPage; $i++) {
            $response = $httpClient->get('https://www.sikayet.com/firma/vitra/'. $i);
            ...
            $articles = $xpath->evaluate('/html/body/div[2]/div/div/div[2]/div[1]/div[3]/div/ul/li');

            foreach ($articles as  $index => $article) {
                $titles = $xpath->evaluate('/html/body/div[2]/div/div/div[2]/div[1]/div[3]/div/ul/li['. $index .']/div[2]/div/a');

                foreach ($titles as $key => $title) {
                    $this->scrape_page_detail($title->getAttribute('href'), $title->textContent);
                }
            }
        }
    }
    ...
    public function scrape_page_detail($url, $title)
    {
        ...
        $articles = $xpath->evaluate('/html/body/div[3]/div/div/div[2]/div[1]/div[2]/p');
